...
def parse_compare(args):
    ...
    group.add_argument("-l", "--league", dest="leagues", action="append", nargs="*", 
        choices=available_datasets)
    ...
    result = vars(parser.parse_args(args))
    result["leagues"] = [item for sublist in result["leagues"] for item in sublist]
    compare_stat_between_datasets(db_dir, result["leagues"], result["team"], result["stat"])
def parse_distribution(args):
    parser = ArgumentParser()
    parser.add_argument("-l", "--leagues", dest="leagues", nargs="*", action="append",
    ...
    if len(result["leagues"]) != 2:
        print("Must provide exactly two leagues to create a distribution.")
        sys.exit()
    else:
        compare_distribution(db_dir, result["leagues"][0], result["leagues"][1], result["team"], result["stat"])
